...
struct FpmmTradeEvent {
  ...
  int64_t side, outcome_index, usdc_amount, token_amount, fee;
};
...
struct FpmmFundingEvent {
  ...
  int64_t side;
  std::string amounts;
  int64_t collateral_from_fee_pool, shares;
};
...
struct OrderFilledEvent {
  ...
  int64_t maker_amount, taker_amount, fee;
};
