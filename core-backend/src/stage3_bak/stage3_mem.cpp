...
json StageSync::memory_breakdown() const {
  ...
  int64_t cache_snapshots_bytes = 0;
  {
    ...
    cache_addr_bytes = core::mem::estimate_string_extra(user_query_cache_state_.addr_lower);
    cache_timeline_bytes = core::mem::estimate_vector_plain(user_query_cache_state_.timeline);
    cache_snapshots_bytes = core::mem::estimate_vector(user_query_cache_state_.snapshots, [](const UserQueryCacheState::PositionSnapshot &s) {
      return core::mem::estimate_vector_plain(s.positions);
    });
  }

  RuntimeMemoryProbe probe_copy;
  {
