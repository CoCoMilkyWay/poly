...
public:
  ApiServer(asio::io_context &ioc, Database &stage1_db, Database &stage2_db, stage3::StageSync &stage3,
            unsigned short port, ApiSession::Stage0Getter stage0_getter, ApiSession::Stage1Getter stage1_getter,
      ...
      : ioc_(ioc), acceptor_(ioc, tcp::endpoint(tcp::v4(), port)), stage1_db_(stage1_db), stage2_db_(stage2_db),
        stage3_(stage3), stage0_getter_(std::move(stage0_getter)), stage1_getter_(std::move(stage1_getter)),
    ...
    acceptor_.async_accept([this](beast::error_code ec, tcp::socket socket) {
      if (!ec) {
        std::make_shared<ApiSession>(std::move(socket), stage1_db_, stage2_db_, stage3_,
                                     stage0_getter_, stage1_getter_, stage2_getter_, stage3_getter_)->run();
      }
      do_accept();
    });
  }
