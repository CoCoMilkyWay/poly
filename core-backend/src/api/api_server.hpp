...
class ApiServer {
public:
  ApiServer(asio::io_context &ioc, Database &stage0_db, Database &stage1_db, Database &stage2_db,
      ...
      : ioc_(ioc), acceptor_(ioc, tcp::endpoint(tcp::v4(), port)), stage0_db_(stage0_db), stage1_db_(stage1_db),
        stage2_db_(stage2_db), stage3_(stage3), stage0_getter_(std::move(stage0_getter)),
...
private:
  void do_accept() {
    ...
    acceptor_.async_accept([this](beast::error_code ec, tcp::socket socket) {
      if (!ec) {
        std::make_shared<ApiSession>(std::move(socket), stage0_db_, stage1_db_, stage2_db_, stage3_,
