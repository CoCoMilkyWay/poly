...
class ApiServer {
public:
  ApiServer(asio::io_context &ioc, Database &db, SyncRepair &sync_repair,unsigned short port)
      : ioc_(ioc), acceptor_(ioc, tcp::endpoint(tcp::v4(), port)), db_(db),sync_repair_(sync_repair) {
