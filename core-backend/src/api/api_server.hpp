...
class ApiServer {
public:
  ApiServer(asio::io_context &ioc, Database &db, stage2::EventSync &event_sync,
            stage3::PnlEngine &pnl_engine, unsigned short port,
            ApiSession::SyncStatusGetter sync_getter = nullptr)
      : ioc_(ioc), acceptor_(ioc, tcp::endpoint(tcp::v4(), port)), db_(db),
        event_sync_(event_sync), pnl_engine_(pnl_engine), sync_getter_(std::move(sync_getter)) {
...
private:
  void do_accept() {
    acceptor_.async_accept([this](beast::error_code ec, tcp::socket socket) {
      if (!ec) {
        std::make_shared<ApiSession>(std::move(socket), db_, event_sync_, pnl_engine_, sync_getter_)->run();
      }
      do_accept();
    });
  }
