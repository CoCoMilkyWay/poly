...
QuerySync::QuerySync(const Config &config, Database &stage1_db, Database &stage0_db,
...
void QuerySync::start(asio::io_context &ioc) {
  ioc_ = &ioc;
...
QuerySync::Status QuerySync::status() const {
  std::lock_guard<std::mutex> lock(status_mutex_);
  return sync_;
}

void QuerySync::schedule_sync(int delay_seconds) {
  if (stop_requested_) {
    return;
  }
  assert(ioc_ != nullptr);
...
void QuerySync::refresh_status_locked(int64_t head_block, int64_t cursor, bool syncing) {
...
void QuerySync::record_commit_locked(int64_t cursor) {
...
void QuerySync::load_known_conditions() {
...
void QuerySync::ensure_cursor_floor() {
...
QuerySync::SeedScanBatch QuerySync::load_seed_scan_batch(int64_t start_block, int64_t head_block,
...
int64_t QuerySync::get_scan_cursor() {
  auto conn = stage0_db_.create_connection();
...
void QuerySync::set_scan_cursor_in_txn(duckdb::Connection &conn, int64_t block) {
