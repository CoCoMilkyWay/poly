...
WordPieceTokenizer::TokenIds WordPieceTokenizer::encode(const std::string &text, int max_len) const {
...
std::vector<int> WordPieceTokenizer::tokenize(const std::string &text) const {
...
std::vector<int> WordPieceTokenizer::wordpiece_tokenize(const std::string &word) const {
  std::vector<int> tokens;
  size_t start = 0;
...
std::vector<std::vector<float>> OnnxEmbedder::embed(const std::vector<WordPieceTokenizer::TokenIds> &batch) const {
  if (batch.empty()) {
    return {};
  }
...
const std::string &OnnxEmbedder::device_name() const {
  return device_name_;
}
...
TaggerModelInfo detect_tagger_model_info(const std::string &model_dir) {
  ...
  info.model_size_text = detail::detect_model_size_text(info.model_path);
  return info;
}
...
Tagger::Impl::Impl(const std::string &model_dir, const std::string &tag_md_path)
    : tokenizer(model_dir + "/vocab.txt"),
      embedder(model_dir + "/model.onnx"),
      labels(detail::parse_tag_md(tag_md_path)) {
  std::vector<detail::WordPieceTokenizer::TokenIds> label_tokens;
...
std::vector<TagResult> Tagger::tag_batch(const std::vector<std::string> &questions) const {
  ...
  std::vector<detail::WordPieceTokenizer::TokenIds> tokens;
