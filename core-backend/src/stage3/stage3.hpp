...
static_assert(MAX_SHARPE_BUCKETS % STAGE3_SYNC_SHARD_COUNT == 0);
static_assert(MAX_SHARPE_POINTS % STAGE3_SYNC_SHARD_COUNT == 0);
...
constexpr uint32_t SHARPE_BUCKETS_PER_SYNC_SHARD = static_cast<uint32_t>(MAX_SHARPE_BUCKETS / STAGE3_SYNC_SHARD_COUNT);
constexpr uint32_t SHARPE_POINTS_PER_SYNC_SHARD = static_cast<uint32_t>(MAX_SHARPE_POINTS / STAGE3_SYNC_SHARD_COUNT);
...
struct SharpeBucket { // 40B
  ...
  int64_t close_exposure; // bucket 末尾 exposure
  uint32_t point_head;    // -> sharpe_point_pool 链表头
  uint32_t point_tail;    // -> sharpe_point_pool 链表尾
  uint32_t next;          // 同用户下一个 bucket / free list
};

static_assert(sizeof(SharpeBucket) == 40);
...
static_assert(sizeof(SharpePoint) == 24);
...
struct UserBlock { // 128B
  ...
  uint32_t sharpe_bucket_head; // 4B
...
struct Stage3Runtime {
  ...
  SharpeBucket *sharpe_bucket_pool;
  SharpePoint *sharpe_point_pool;
  UserBlock *users;
...
inline uint64_t sharpe_bucket_pool_used_total(const StoreHeader *header) {
  return sum_shard_used(header->sharpe_bucket_pool_used);
}

inline uint64_t sharpe_point_pool_used_total(const StoreHeader *header) {
  return sum_shard_used(header->sharpe_point_pool_used);
}
...
inline uint32_t sharpe_bucket_shard_from_index(uint32_t idx) {
  return idx / SHARPE_BUCKETS_PER_SYNC_SHARD;
}

inline uint32_t sharpe_bucket_global_from_local(uint32_t shard, uint32_t local_idx) {
  return shard * SHARPE_BUCKETS_PER_SYNC_SHARD + local_idx;
}

inline uint32_t sharpe_point_shard_from_index(uint32_t idx) {
  return idx / SHARPE_POINTS_PER_SYNC_SHARD;
}

inline uint32_t sharpe_point_global_from_local(uint32_t shard, uint32_t local_idx) {
  return shard * SHARPE_POINTS_PER_SYNC_SHARD + local_idx;
}
